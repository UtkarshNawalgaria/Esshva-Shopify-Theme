class CartNotification extends HTMLElement {
  ...
  renderContentsOnLoad() {
    var _this = this;
    fetch(
      `${window.Shopify.routes.root}?sections=cart-notification-content,cart-notification-crosssell-products`
    )
  ...
  updateContent(parsedStateSections) {
    ...
    this.getSectionsToRender().forEach((section) => {
      const parsedStateHtml = new DOMParser().parseFromString(
        parsedStateSections[section.id],
        "text/html"
      );
      if (
        section.id == "cart-notification-content" &&
        section.hasOwnProperty("elements") &&
        document
          .getElementById(section.id)
          .querySelector("[data-mini-cart-wrapper]") &&
        parsedStateHtml.querySelector("[data-mini-cart-wrapper]")
      ) {
        section["elements"].forEach(function (elem) {
          const content = _this.getSectionInnerHTML(
            parsedStateSections[section.id],
            elem
          );
          if (content != "") {
            ...
            document.getElementById(section.id).querySelector(elem).innerHTML =
              content;
          } else {
            document
              .getElementById(section.id)
        ...
        document.getElementById(section.id).innerHTML =
          this.getSectionInnerHTML(parsedStateSections[section.id]);
      }
    });
  ...
  getSectionInnerHTML(html, selector = ".shopify-section") {
    ...
    return parsedHtml.querySelector(selector)
      ? parsedHtml.querySelector(selector).innerHTML
      : "";
  }
        ...
        (e) => {
          ...
          if (action == "shipping") {
            ...
            (e.zip = document.querySelector("#address_zip").value || ""),
              (e.country =
                document.querySelector("#address_country").value || ""),
              (e.province =
        ...
        if (parsedState && parsedState.shipping_rates) {
          if (parsedState.shipping_rates.length > 0) {
            ...
            p.innerHTML =
              `<strong>` +
              "We found " +
              parsedState.shipping_rates.length +
              " shipping rate(s) for your Postal/ZIP code:" +
              `<strong>`;
